  ...
  useEffect(() => {
    ...
    const checkProgress = async () => {
      try {
        ...
        // If intervention not completed, redirect
        if (!data.progress?.interventionCompleted) {
          navigate('/intervention', { replace: true })
          return
        }

        // If posttest already completed, load existing answers
        if (data.progress?.posttestCompleted && data.posttestAnswers?.length > 0) {
          const existingAnswers = {}
          data.posttestAnswers.forEach(item => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${server}/api/progress/posttest`,
        { answers },
        { withCredentials: true }
      )

      toast.success(`Posttest submitted! Score: ${data.posttestScore}/20`)
      navigate('/posttest-result', { replace: true, state: { score: data.posttestScore } })
    } catch (error) {
        ...
        <div className="bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-lg sm:shadow-xl p-4 xs:p-6 sm:p-8 md:p-10">
          <div className="flex items-center justify-between mb-4 xs:mb-6">
            <h1 className="text-2xl xs:text-3xl sm:text-4xl font-bold text-gray-800">
              <span className="text-gradient">Posttest</span>
            </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 xs:space-y-5 sm:space-y-6">
            ...
            >
              {submitting ? 'Submitting...' : 'Submit Posttest'}
            </button>
          </form>
        </div>
      </div>
