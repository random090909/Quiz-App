  ...
  useEffect(() => {
    ...
    if (score === null) {
      axios.get(`${server}/api/progress/results/me`, { withCredentials: true })
        .then(({ data }) => setScore(data.pretestScore ?? 0))
        .catch(() => setScore(0))
    }
  }, [navigate, score])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-indigo-50 py-8 px-4">
      <div className="max-w-xl mx-auto bg-white rounded-2xl shadow-lg p-8 text-center">
        <h1 className="text-3xl font-bold text-gray-800 mb-4">Pretest Result</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          {/* pass state so Intervention knows this navigation came from pretest (reattempt) */}
          <button
            onClick={() => navigate('/intervention', { replace: true, state: { from: 'pretest', reattempt: true } })}
